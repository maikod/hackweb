...
class Accounts extends DB{
    ...
    function requestPosts($data){        
        $db = $this->conn;                            
        //dev10n
        $sql = 'SELECT posts.id, posts.cat, posts.image, posts.title, posts.subtitle, posts.description, posts.video_link
        FROM brah_posts AS posts     
        WHERE status = 1                   
        ORDER BY posts.ord2 DESC
        LIMIT ?, ?';
        $stmt = $db->prepare($sql);
        $stmt->bind_param('dd',$data->origin,$data->increment);
        $stmt->execute();
        $stmt->bind_result($id,$cat,$image,$title,$subtitle,$description,$video_link);
        $result = array();
        $video_num = 0;
        while($stmt->fetch()){
            if($cat == 1){
                echo '<div class="cell">
                    <img class="img-content" src="'.$image.'" />
                    <a class="overview-img-link" data-fancybox="gallery" href="'.$image.'" val="'.$id.'">
            ...
            }else if($cat == 2){ //youtube video
                $video_num++;
                $video_link = substr($video_link, strpos($video_link, "?") + 3);                    
                echo '<div class="cell cell-video">
                    <iframe class="video-player" video_num="'.$id.'" id="player-'.$id.'" src="https://www.youtube.com/embed/'.$video_link.'?rel=0&amp;controls=0&amp;showinfo=0&amp;autoplay=0&amp;vq=hd1080&amp;enablejsapi=1&amp;fs=0&amp;playsinline=1" frameborder="0" allowfullscreen="0" fs="0" playsinline="1"></iframe>                                        
                    <a data-fancybox href="https://www.youtube.com/watch?v='.$video_link.'&amp;autoplay=1&amp;rel=0&amp;controls=1&amp;showinfo=0">
                        <div class="cell-wrapper">
                            <div class="wrapper-text">
                                <div class="wrapper-title">                            
                                '.$title.'
                ...
                echo '<div class="cell cell-my_video">
                <video class="video-content" playsinline muted loop>
...
if(isset($action)){        
    $api = new Accounts;            
    if($action == "requestPosts"){
        $api->requestPosts($data);
    }
